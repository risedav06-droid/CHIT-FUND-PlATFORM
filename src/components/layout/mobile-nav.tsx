      ...
      <div className="mx-auto grid max-w-7xl grid-cols-4 gap-2">
        {items.slice(0, 4).map((item) => (
          <Link
            ...
            href={item.href}
            className={cn(
              "relative rounded-2xl border border-border bg-surface/60 px-3 py-3 text-center text-xs font-semibold text-foreground transition hover:border-brand hover:text-brand",
            )}
          >
            {item.title}
